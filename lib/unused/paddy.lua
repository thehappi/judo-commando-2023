 ...
 * along with this program. If not, see <http://www.gnu.org/licenses/>.
 ...
 paddy.dpad.x=15
 paddy.dpad.y=love.graphics.getHeight()-15-paddy.dpad.h
 ...
 paddy.dpad.buttons={
     { name="up",   x=paddy.buttonw, y=0, w=paddy.buttonw, h=paddy.buttonh*1.18 },
     { name="left", x=0, y=paddy.buttonh, w=paddy.buttonw*1.5, h=paddy.buttonh },
     { name="right",x=paddy.buttonw*3-paddy.buttonw*1.5 , y=paddy.buttonh, w=paddy.buttonw*1.5, h=paddy.buttonh },
     { name="down", x=paddy.buttonw, y=paddy.buttonh*3-paddy.buttonh*1.18, w=paddy.buttonw, h=paddy.buttonh*1.18 },
 }
 ...
 paddy.buttons.buttons={
     { name="b", x=paddy.buttonw, y=0, w=paddy.buttonw, h=paddy.buttonh },
     -- { name="x", x=0, y=paddy.buttonh },
     -- { name="b", x=paddy.buttonw*2, y=paddy.buttonh },
     { name="a", x=paddy.buttonw*1.5, y=paddy.buttonh, w=paddy.buttonw, h=paddy.buttonh },
 }
 ...
 function paddy.draw()
     ...
     for _,widget in ipairs(paddy.widgets) do
         ...
         for _,button in ipairs(widget.buttons) do
                 love.graphics.setColor(0.607,0.607,0.607, button.isDown and 1 or 0.784)
                 love.graphics.rectangle(button.isDown and "fill" or "line", 
                     button.x+widget.padding, 
                     button.y+widget.padding, 
                     button.w-widget.padding*2, 
                     button.h-widget.padding*2,
                     10
                 )
             -- end
      ...
      for _,widget in ipairs(paddy.widgets) do
         for _,button in ipairs(widget.buttons) do
             ...
             for _,id in ipairs(paddy.touched) do	
                 ...
                 and tx <= widget.x+button.x+button.w
                 and ty >= widget.y+button.y 
                 and ty <= widget.y+button.y+button.h then
                     button.isDown=true
                 end
             end
         end
